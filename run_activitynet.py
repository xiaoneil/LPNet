...
parser.add_argument("--root", type=str, default='data/ActivityNet', help="root directory for store raw data")
...
if configs.mode.lower() == "prepro":
    ...
    video_feature_path = os.path.join(configs.root, "activitynet_features_{}".format(configs.feature))
    ...
    with tf.Graph().as_default() as graph:
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            freeze_bbox_var_list = [
                t for t in trainable_vars
                if not t.name.startswith(u'proposal_box')
            ]
            bbox_var_list = [
                t for t in trainable_vars if t.name.startswith(u'proposal_box')
            ]
            train_op1 = optimizer.minimize(model.reg_loss,
                                           global_step=model.global_step,
                                           var_list=freeze_bbox_var_list)
            train_op2 = optimizer.minimize(model.my_loss,
                                           global_step=model.global_step,
                                           var_list=bbox_var_list)
            ...
            for epoch in range(configs.epochs):
                for data in tqdm(batch_iter(train_set, video_features, configs.batch_size, configs.extend, True, True),
                    ...
                    _, _, loss, rloss, iloss, lloss, global_step = sess.run([train_op1, train_op2, model.my_loss, model.reg_loss, model.iou_loss, model.l1_loss, model.global_step], feed_dict=feed_dict)
