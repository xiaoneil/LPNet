...
from utils.prepro_charades import prepro_charades
...
parser.add_argument("--feature", type=str, default='c3d', help="[finetune | raw]")
parser.add_argument("--root", type=str, default='data/Charades', help="root directory for store raw data")
...
parser.add_argument("--max_position_length", type=int, default=256, help="max position length")
...
parser.add_argument("--eval_period", type=int, default=1000, help="evaluation period")
...
# specify the dataset and model directory
if configs.home_dir is None:
    configs.home_dir = "ckpt/charades_{}_{}".format(configs.feature, configs.max_position_length)
configs.save_dir = "datasets/charades_{}/{}".format(configs.feature, configs.max_position_length)
...
if configs.mode.lower() == "prepro":
    ...
    # demo_set =load_json(os.path.join(configs.save_dir, "demo_set.json"))
    ...
    with tf.Graph().as_default() as graph:
        ...
        with tf.Session(config=sess_config) as sess:
            learning_rate = tf.train.exponential_decay(learning_rate=configs.init_lr, global_step=model.global_step, decay_steps=100000, decay_rate=0.9, staircase=True)
            ...
            saver_all = tf.train.Saver(max_to_keep=5)
            ...
            for epoch in range(configs.epochs):
                for data in tqdm(batch_iter(train_set, video_features, configs.batch_size, configs.extend, train=True, shuffle=True),
                                 total=num_train_batches, desc="Epoch %d / %d" % (epoch + 1, configs.epochs)):

                    # run the model
                    feed_dict = get_feed_dict(data, model, configs.drop_rate)

                    _, _, train, hloss, lloss, loss, rloss, iloss, l1loss, global_step = sess.run([train_op1, train_op2, model.train, model.highlight_loss, model.loss, model.my_loss, model.reg_loss, model.iou_loss, model.l1_loss, model.global_step], feed_dict=feed_dict)
                    # print(train)
                    if global_step % configs.period == 0:
                        write_tf_summary(writer, [("train/my_loss", loss),("train/reg_loss", rloss),("train/iou_loss", iloss),("train/l1_loss", l1loss),("train/loss", lloss),("train/highlight_loss", hloss)], global_step)

                    if (global_step + 1)% num_train_batches == 0:
    ...
    with tf.Graph().as_default() as graph:
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            saver.restore(
                sess, tf.train.latest_checkpoint(model_dir)) 
